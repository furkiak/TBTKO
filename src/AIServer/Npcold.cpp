...
{
	Packet result(MOVE_RESULT, uint8(NORMAL));
...
time_t CNpc::NpcTracing()
{
	...
	if (!IsMovingEnd())
		SendMoveResult(GetX(), GetY(), GetZ());//, (float)(m_fSecForRealMoveMetor / ((double)m_sSpeed / 1000)));
	else
...
void CNpc::MoveAttack()
{
	...
	if (IsMovingEnd())
		...
		SendMoveResult(m_fPrevX, m_fPrevY, m_fPrevZ, (float)(m_fSecForRealMoveMetor / ((double)m_sSpeed / 1.1f)));
	RegisterRegion(GetX(), GetZ());
