...
public:
	...
	//EvaneScence tarafýndan Ko-foxa Yapýlmýstýr
	uint16	m_GenieTime;
...
public:
	...
	INLINE bool isInEventZone() {  return GetZoneID() == ZONE_BORDER_DEFENSE_WAR || GetZoneID() == ZONE_CHAOS_DUNGEON || GetZoneID() == ZONE_JURAD_MOUNTAIN; }
	...
	*/
	...
	INLINE std::string GetMemo() { return m_strMemo; }
	...
	bool GoldLose(uint32 gold, bool LbSendPacket = true);
	...
	void HandleUserInfoAll(Packet & pkt);
	void HandleUserInfoShow(Packet & pkt);
	void HandleUserInfo(Packet & pkt);
	void HandleUserInfoSystem(Packet & pkt);
   ...
   void ExpSealOn(Packet & pkt);
	...
	COMMAND_HANDLER(HandleWar7OpenCommand);
	...
	//Genie
	void HandleGenieSystem(Packet & pkt);
	...
	//void TerminationTime(bool bSendAll = false, CUser *pUser = nullptr, uint8 ZoneID = 0);
