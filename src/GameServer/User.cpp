...
void CUser::SendMyInfo()
{
	...
	result << uint8(0) << uint32(0) << uint8(0) << uint16(GetGenieTime());
#endif
#if (__VERSION >= 2010) // V2 ise gerekli bilgileri de ekle
	result << uint8(GetGenieStatus());

	for(int i=0; i<STAT_COUNT; i++)
		result <<  m_bRebStats[i];
	...
	result << uint8(0)
		...
		<< uint16(0)
		<< uint8(0) // ??
		<< uint16(GetAchieveTitle()) // Achieve Title
		<< uint16(GetAchieveSkillTitle())
		<< uint16(GetAchieveSkillTitle()); // ??
#endif
...
void CUser::SetZoneAbilityChange(uint16 sNewZone)
{
	...
	if (sNewZone == ZONE_BORDER_DEFENSE_WAR)
		g_pMain->TerminationTime(false, this, (uint8)sNewZone);
	if (sNewZone == ZONE_CHAOS_DUNGEON || sNewZone == 81 || sNewZone == 82 || sNewZone == 83)
		g_pMain->TerminationTime(false, this, (uint8)sNewZone);
...
void CUser::BundleOpenReq(Packet & pkt)
{
	...
	result << bundle_index << uint8(0x01);
...
void CUser::UserLookChange(int pos, int itemid, int durability)
{
	...
	if (g_pMain->pTempleEvent.ActiveEvent != -1 && isEventUser())
...
void CUser::TopSendNotice()
{
	...
	//neeed mykopackettoo  ofset nowl ofset yok */
}
...
void CUser::BlinkStart()
{
	...
	if (isInPKZone() || isGM() || isInEventZone())
		return;
		...
		else
		{
			...
			else
			{
				...
				{   
					 ...
					 if (g_pMain->pTempleEvent.ElMoradUserCount > 59 || g_pMain->pTempleEvent.KarusUserCount > 59)
				...
				else
				{
					...
					else if (GetZoneID() == ZONE_DELOS && g_pMain->m_byBattleOpen == CLAN_BATTLE)
					{	
						noticeType = DeathNotice;
						pUser->LoyaltyChange(GetID());
					}
...
void CUser::HandleMiningSystem(Packet & pkt)
{
	...
	case FishingStop:
		...
		default:
printf("Unhandled UserInfo system opcode: %X\n", opcode);
        ...
        else if (rate >= 50 && rate <= 80 && random <= 5000)
        {
            if (pTable->m_iNum == MATTOCK) { //Normal Mattock İle Verilicekler
                ...
                switch(MiningItemRandomGroup) {
                    ...
                    default:
						GiveItem(g_pMain->m_sling); 
                    break;
                }
            } else if(pTable->m_iNum == GOLDEN_MATTOCK) {// Golden Mattock ile verilicekler...
                int MiningItemRandomGroup = 0;
                srand((unsigned)time(0));
                MiningItemRandomGroup = rand() % 20;
                switch(MiningItemRandomGroup) {
 ...
 void CUser::HandleFishingStop(Packet & pkt)
{
 ...
 void CUser::HandleGenieSystem(Packet & pkt)
{
	uint8 opcode;
	pkt >> opcode;
...
void CUser::HandleUserInfoSystem(Packet & pkt)
{
	uint8 opcode;
	pkt >> opcode;
...
Packet result(WIZ_USER_INFO,uint8(UserInfoShow));
	...
	result <<uint8(1)<<uint8(0)<< GetName();
...
{
Packet result(WIZ_USER_INFO, uint8(3));
	short user_count = pkt.read<uint16>(), online_count = 0;
	if (user_count > 1000)
		user_count = 1000;
	pkt.SByte(); 
