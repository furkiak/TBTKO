...
void CNpc::MoveResult(float fPosX, float fPosY, float fPosZ, float fSpeed)
{
	...
	result << GetID() << GetSPosX() << GetSPosZ() << GetSPosY() << uint16(fSpeed * 0x0f);
	SendToRegion(&result);
}
...
void CNpc::OnDeathProcess(Unit *pKiller)
{
	...
	else if (m_bMonster)
	{
		...
		{
			if (pUser->isInPKZone() || pUser->GetZoneID() == ZONE_JURAD_MOUNTAIN)
		...
		else if (m_tNpcType == NPC_KJWAR && pUser->isInEventZone()) 
		{
			uint16 KO = 360;
			Packet result1(WIZ_CAPTURE, uint8(0x05));
			result1 << pUser->GetNation() << GetName().c_str();
			Packet result(WIZ_CAPTURE, uint8(0x04));
			result << pUser->GetNation() << KO;
			g_pMain->Send_Zone(&result1, GetZoneID(), nullptr, Nation::ALL);
			...
			if (g_pMain->m_sElmoBdw >= 7 && pUser->GetNation() == ELMORAD || g_pMain->m_sKarusBdw >= 7 && pUser->GetNation() == KARUS)	
				g_pMain->TempleEventFinish();
			
			g_pMain->m_nPVPMonumentNation[GetZoneID()] = pUser->GetNation();
			g_pMain->NpcUpdate(GetProtoID(), m_bMonster, 4 , pUser->GetNation() == KARUS ? BDW_KARUS_MONUMENT_SPID  : BDW_ELMO_MONUMENT_SPID );
		}
