					   ...
					   tstring & strGameDSN, tstring & strGameUID, tstring & strGamePWD)
{
	...
	if (!m_GameDB->Connect(strGameDSN, strGameUID, strGamePWD))
	{
...
bool CDBAgent::LoadUserData(string & strAccountID, string & strCharID, CUser *pUser)
{
	...
	char	strItem[INVENTORY_TOTAL * 8] = {0}, strSerial[INVENTORY_TOTAL * 8] = {0}, strItemEx[INVENTORY_TOTAL * 4] = {0}, strQuest[QUEST_ARRAY_SIZE];
...
bool CDBAgent::UpdateUser(string & strCharID, UserUpdateType type, CUser *pUser)
{
	...
	if (!dbCommand->Execute(string_format(_T("{CALL UPDATE_USER_DATA("
		...
		pUser->m_iMannerPoint, pUser->m_iLoyaltyMonthly, pUser->m_strMemo)))
	{
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
...
uint16 CDBAgent::LoadKnightsAllMembers(uint16 sClanID, Packet & result)
{
	...
	if (!dbCommand->Execute(string_format(_T("SELECT strUserID, Fame, [Level], strMemo, Class FROM USERDATA WHERE Knights = %d"), sClanID)))
		ReportSQLError(m_GameDB->GetError());
	...
	do
	{
		string strCharID;string bMemo; uint16 sClass; uint8 bFame, bLevel;
		dbCommand->FetchString(1, strCharID);
		dbCommand->FetchByte(2, bFame);
		dbCommand->FetchByte(3, bLevel);
		dbCommand->FetchUInt16(4, sClass);
		dbCommand->FetchString(5, bMemo);

		result << strCharID << bFame << bLevel << sClass << bMemo 
