...

{
	...
	case NPC_IDK:
		result<<uint8(1)<<uint8(2);
			break;

	default:
		printf("Unhandled Npc system Type: %X\n", GetType);
		ClientEvent(sNpcID);
...
send_packet:
 ...
 if (!bSuccess)
  ...
  result << pTable->m_bSellingGroup <<UINT8(0)<<m_iGold << transactionPrice; // price bought or sold for
 Send(&result);
}
