...
bool CGameServerDlg::Startup()
{
	...
	if (!g_pMain->m_socketMgr.Listen(ServerIPAddress, m_GameServerPort, MAX_USER))
	{
...
void CGameServerDlg::GetTimeFromIni()
{
	...
	bool bMarsEnabled = ini.GetBool("ODBC", "MARS", true);
	...
	m_Drop1 = ini.GetInt("SETTINGS","MYSTERIOUS_ORE", 399210000);
	m_Drop2 = ini.GetInt("SETTINGS","MYSTERIOUS_ORE_1", 399210000);
	m_Drop3 = ini.GetInt("SETTINGS","MYSTERIOUS_ORE_2", 399210000);
	m_Drop4 = ini.GetInt("SETTINGS","MYSTERIOUS_ORE_3", 399210000);
	m_Drop5 = ini.GetInt("SETTINGS","MYSTERIOUS_ORE_4", 399210000);
	m_gDrop1 = ini.GetInt("SETTINGS","MYSTERIOUS_GOLD_ORE", 399210000);
	m_gDrop2 = ini.GetInt("SETTINGS","MYSTERIOUS_GOLD_ORE_1", 399210000);
	m_gDrop3 = ini.GetInt("SETTINGS","MYSTERIOUS_GOLD_ORE_2", 399210000);
	m_gDrop4 = ini.GetInt("SETTINGS","MYSTERIOUS_GOLD_ORE_3", 399210000);
	m_gDrop5 = ini.GetInt("SETTINGS","MYSTERIOUS_GOLD_ORE_4", 399210000);
	m_sling = ini.GetInt("SETTINGS","SLING", 389043000);

	//Fishing
	f_Drop1 = ini.GetInt("SETTINGS","FISHING_ROD1", 399127000);
	f_Drop2 = ini.GetInt("SETTINGS","FISHING_ROD2", 399128000);
	f_Drop3 = ini.GetInt("SETTINGS","FISHING_ROD3", 399129000);
	f_gDrop1 = ini.GetInt("SETTINGS","FISHING_GOLDEN_ROD1", 399129000);
	f_gDrop2 = ini.GetInt("SETTINGS","FISHING_GOLDEN_ROD2", 399128000);
	f_gDrop3 = ini.GetInt("SETTINGS","FISHING_GOLDEN_ROD3", 399127000);


	m_MerchantStatus = ini.GetInt("STATUS","MERCHANT_STATUS", 0);
	m_TradeStatus = ini.GetInt("STATUS","TRADE_STATUS", 0);
	...
	/*for (int i = 0; i < JURAD_MOUNTAIN_EVENT_COUNT; i++)
		m_nJuraidTime[i] = ini.GetInt("JURAID",string_format("START_TIME%d",i+1).c_str(), 0);*/
	...
	m_nPVPMonumentNation[ZONE_BORDER_DEFENSE_WAR] = 0;
...
uint32 CGameServerDlg::Timer_CheckGameEvents(void * lpParam)
{
	while (g_bRunning)
	{
		...
		g_pMain->SnowBattleZoneOpenTimer();
											 ...
											 uint8 nation, uint8 seekingPartyOptions, uint16 nEventRoom)
{
	...
	foreach (itr, sessMap)
	{
		...
		if (nEventRoom > 0 && nEventRoom != pUser->GetEventRoom() && pUser->GetZoneID () == ZONE_OLD_MORADON)
			continue;
...
void CGameServerDlg::GetRegionNpcIn(C3DMap *pMap, uint16 region_x, uint16 region_z, Packet & pkt, uint16 & t_count, uint16 nEventRoom, CUser* pSendUser)
{
	...
	foreach (itr, pRegion->m_RegionNpcArray)
	{
		...
		if (nEventRoom > 0 && nEventRoom != pNpc->m_bEventRoom && pNpc->GetProtoID() != SAW_BLADE_SSID && pNpc->m_bZone == ZONE_OLD_MORADON)
			continue;
...
bool CGameServerDlg::TopLoadNoticeData()
{
	...
	else if (pTempleEvent.ActiveEvent != -1)
	{
		if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_BORDER_DEFENCE_WAR)
		{
			...
			for(int i = 0; i < BORDER_DEFENSE_WAR_EVENT_COUNT; i++)
			{
				...
				{
					BorderDefenseWarTerminationTimeActive();
					pTempleEvent.isAttackable = false;
					break;
				}
...
void CGameServerDlg::JuradTempleEventStart()
{
	Packet resoult(WIZ_SELECT_MSG);
	...
	pTempleEvent.ElMoradUserCount = 0;
	pTempleEvent.AllUserCount = 0;
	resoult << uint16(0x00) << uint8(0x07) << uint64(0x00) << 0x08 << uint8(0x97) << uint16(0x1B) << uint8(0x00) << m_nTempleEventRemainSeconds;		
	//result << m_nTempleEventRemainSeconds;
	g_pMain->Send_All(&resoult,nullptr,Nation::ALL,ALL);
	//g_pMain->Send_All(&result,nullptr,Nation::ALL,ALL);
}
...
void CGameServerDlg::TempleEventFinish(uint16 m_sEventRoom, uint8 m_bNation)
{
	foreach_stlmap (itr, m_TempleEventUserArray)
	{
		...
		if (pRankInfo)
		{
			...
			{
				if (pTempleEvent.ElMoradUserCount >= pTempleEvent.KarusUserCount && pUser->GetNation() == KARUS) 
					pUser->GiveItem(RED_TREASURE_CHEST);
				if (pTempleEvent.KarusUserCount >= pTempleEvent.ElMoradUserCount && pUser->GetNation() == ELMORAD)	
					pUser->GiveItem(RED_TREASURE_CHEST);
...
uint16 CGameServerDlg::GetKnightsAllMembers(uint16 sClanID, Packet & result, uint16 & pktSize, bool bClanLeader)
{
		...
		foreach_array (i, pKnights->m_arKnightsUser)
	{
		...
		if( pCurrent == nullptr )
		{
			if(p->m_bLevel == 0 || p->m_sFame == 0 || p->m_sClass == 0)
...
void CGameServerDlg::BorderDefenseWarTerminationTimeActive()
{
foreach_stlmap (itr, m_TempleEventUserArray)
{
CUser * pUser = GetUserPtr(itr->second->m_socketID);
...
result << uint16(0) << uint8(7) << uint32(0) << uint32(0) << uint8(8) << uint16(0) << uint8(0) << uint8(0x07) << uint8(0x05) << uint8(0x07) << uint16(0);
pUser->Send(&result);

result.clear();
result.Initialize(WIZ_BIFROST);
result << uint8(0x05) << uint8(0x05) << uint8(0x07);
pUser->Send(&result);

result.clear();
result.Initialize(WIZ_EVENT);
result << uint8(0x03) << uint32(0)<< uint32(0) << uint32(0x05008504) << uint8(0) << pUser->GetName();
pUser->SendToRegion(&result, nullptr, pUser->m_bEventRoom);
}
}


void CGameServerDlg::JuraidTerminationTimeActive()
{
foreach_stlmap (itr, m_TempleEventUserArray)
{
CUser * pUser = GetUserPtr(itr->second->m_socketID);
...
Packet result(WIZ_SELECT_MSG);
result << uint16(0x00) << uint8(0x07) << uint64(0x00) << 0x08 << uint8(0x97) << uint16(0x1B) << uint8(0x00);
pUser->Send(&result);
...
result.clear();
result.Initialize(WIZ_EVENT);
result << uint8(0x03) << uint32(0)<< uint32(0) << uint32(0x05008504) << uint8(0);
pUser->SendToRegion(&result, nullptr, pUser->m_bEventRoom);
}
}
...
void CGameServerDlg::ChaosTerminationTimeActive()
{
foreach_stlmap (itr, m_TempleEventUserArray)
{
CUser * pUser = GetUserPtr(itr->second->m_socketID);
...
Packet result(WIZ_SELECT_MSG);
result << uint8(0x05) << uint8(0xAC) << uint8(0x04);
pUser->Send(&result);
