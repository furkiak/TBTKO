...
void CUser::GetUserInfo(Packet & pkt)
{
 ...
 pkt <<uint16(0)<<uint16(0)<<uint16(0) <<uint8(0)<< GetZoneID()<< uint8(-1) << uint8(-1) << uint16(0) << uint16(0) << uint16(0)<<uint16(0)<<uint8(0)<<uint16(0)<<uint16(0)<<uint8(0)<<uint8(1);
}
...
void CUser::ZoneChange(uint16 sNewZone, float x, float z)
{
	...
	/*if (sNewZone == ZONE_DELOS)
	 {
		if (pKnightSiege->sMasterKnights == GetClanID() && isInClan())
			return;
...
{
	Packet result(WIZ_NPC_EVENT, uint8(0x03));
	/*TempleOperations(TEMPLE_EVENT_JOIN);
	result << g_pMain->pTempleEvent.KarusUserCount << uint16(0x00) << g_pMain->pTempleEvent.ElMoradUserCount << uint16(0x00);*/
	result << uint8(0x7D) << uint8(0x00) << uint8(0xC3) << uint8(0x76) << uint16(0x00);
	//20037D00C3760000
