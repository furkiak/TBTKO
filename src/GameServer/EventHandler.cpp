...
void CGameServerDlg::TerminationTime(bool bSendAll, CUser *pUser, uint8 ZoneID)
{
	Packet resoult(WIZ_SELECT_MSG);
	Packet result(WIZ_BIFROST, uint8(MONSTER_SQUARD));
	resoult.SByte();
	uint16 nRemainingTime;// = 1800;

	if (ZoneID == ZONE_JURAD_MOUNTAIN)
	{
		nRemainingTime = 1800;//m_byBattleRemainingTime / 2;
		resoult << uint16(0x00) << uint8(0x07) << uint64(0x00) << 0x08 << uint8(0x97) << uint16(0x1B) << uint8(0x00) << nRemainingTime;
		result << nRemainingTime;
	}
	else if (ZoneID == ZONE_CHAOS_DUNGEON || ZoneID ==  81 || ZoneID == 82 || ZoneID == 83)
	{
		nRemainingTime = 1800;//m_sBifrostRemainingTime;
		resoult << uint16(0x00) << uint8(0x07) << uint64(0x00) << 0x09 << uint8(0x18) << uint16(0x1B) << uint8(0x00) << nRemainingTime;
		result << nRemainingTime;
	}
	...
	else if (bSendAll)
	{
		if (ZoneID == ZONE_JURAD_MOUNTAIN)
		{
			g_pMain->Send_All(&resoult,nullptr,0, ZONE_JURAD_MOUNTAIN);
			g_pMain->Send_All(&result,nullptr, 0, ZONE_JURAD_MOUNTAIN);
		}
		else if (ZoneID == ZONE_CHAOS_DUNGEON)
		{
			g_pMain->Send_All(&resoult,nullptr, 0, ZONE_CHAOS_DUNGEON);
			g_pMain->Send_All(&result,nullptr, 0, ZONE_CHAOS_DUNGEON);
		}
		else if (ZoneID ==  81)
		{
			g_pMain->Send_All(&resoult,nullptr,0, 81);
			g_pMain->Send_All(&result,nullptr, 0, 81);
		}
		else if (ZoneID == 82)
		{
			g_pMain->Send_All(&resoult,nullptr,0, 82);
			g_pMain->Send_All(&result,nullptr, 0, 82);
		}
		else if (ZoneID == 83)
		{
			g_pMain->Send_All(&resoult,nullptr,0, 83);
			g_pMain->Send_All(&result,nullptr, 0, 83);
		}
		else
		{
			g_pMain->Send_All(&resoult,nullptr,0, ZONE_BORDER_DEFENSE_WAR);
			g_pMain->Send_All(&result,nullptr, 0, ZONE_BORDER_DEFENSE_WAR);
		}
	}
}
